...
const Lawyer = ({ lawyer }) => {
    ...
    //availability as per today
    const today = new Date().toLocaleDateString("en-US", { weekday: "long" });
    const isAvailable = availability.includes(today);

    return (
        <div className='border-2 border-gray-300 rounded-2xl p-5 md:p-6 flex flex-col lg:flex-row items-center gap-4 md:gap-6 shadow hover:shadow-lg duration-700 transform hover:-translate-y-1'>
            <div>
            ...
            <div className='space-y-2'>
                ...
                <Link to={`lawyer-details/${license}`}><button className='cursor-pointer text-[#176AE5] border-2 border-[#176AE533] py-1 w-full rounded-4xl'>View Details</button></Link>
            </div>
        </div>
    );
};
